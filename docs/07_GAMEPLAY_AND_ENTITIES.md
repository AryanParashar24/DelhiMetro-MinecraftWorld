...
* **Street Food Vendors:** Placed in Purana Delhi (Heritage district) and near university gates. They serve exceptionally hygienic food and can be configured via datapack to trade emeralds for custom food items (e.g., "Chai" potions, Cooked Mutton).
* **Corporate Workers:** Suited zombies/villagers (using custom heads/leather armor) placed in office lobbies in the Financial District.
...
* **The Heritage Museum Station:** Located deep on the Brown Line between two active stations. It is brilliantly lit, impeccably preserved, and contains a hidden lore book detailing the rich history of the tunnel project.
