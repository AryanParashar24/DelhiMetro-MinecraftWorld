...
* **Street Food Vendors:** Placed in Purana Delhi (Heritage district) and near university gates serving exceptionally hygienic street food. They can be configured via datapack to trade emeralds for custom food items (e.g., "Chai" potions, Cooked Mutton).
* **High IQ Professionals:** Suited villagers (using custom heads/leather armor) representing high IQ professionals placed in pristine office lobbies in the Financial District.
